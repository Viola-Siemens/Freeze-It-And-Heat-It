...
public record AddPotionEffectsData(List<MobEffectInstance> effects, boolean disable, Optional<UUID> owner, String potionType) implements IPouchedFoodData {
	...
	public static final Codec<AddPotionEffectsData> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
					...
					UUIDUtil.CODEC.optionalFieldOf("owner").forGetter(AddPotionEffectsData::owner),
	...
	public void modifyStack(ItemStack itemStack) {
		if(!this.effects.isEmpty()) {
			...
			nbt.put(TAG_EFFECTS, getTagFromEffects(this.effects));
			if(this.disable) {
	...
	public static AddPotionEffectsData fromStackNBT(CompoundTag nbt) {
		if(nbt.contains(TAG_EFFECTS, Tag.TAG_LIST)) {
			...
			return new AddPotionEffectsData(
					IPouchedFoodData.MOB_EFFECT_INSTANCE_CODEC.listOf().parse(NbtOps.INSTANCE, nbt.getList(TAG_EFFECTS, Tag.TAG_COMPOUND)).getOrThrow(false, FIAHILogger::error),
					disable, owner, potionType
			);
		}
		return null;
	}

	static ListTag getTagFromEffects(List<MobEffectInstance> effects) {
