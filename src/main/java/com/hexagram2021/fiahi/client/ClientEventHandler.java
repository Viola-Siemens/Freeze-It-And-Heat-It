	...
	public static void onToolTipShow(ItemTooltipEvent event) {
		...
		if(IFrozenRottenFood.canBeFrozenRotten(itemStack)) {
			...
			if(nbt != null && nbt.contains("temperature", Tag.TAG_ANY_NUMERIC)) {
				event.getToolTip().add(Component.translatable("gui.fiahi.count.description", FoodPouchItem.getItemCount(nbt)));
				event.getToolTip().add(Component.translatable("item.fiahi.temperature.description", (int)nbt.getDouble("temperature")));
			}
		}
	}
